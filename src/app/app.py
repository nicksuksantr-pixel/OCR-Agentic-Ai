...
APP_VERSION = "v0.2.0"  # carry rule: each place 0–9, carry at 9 (v0.1.5 → v0.2.0 redesign)
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.watcher = InboxWatcher(
            self.settings,
            on_event=lambda msg: self.after(0, self.log, "WATCH", msg),
        ...
        self.api = ApiServer(self.settings, APP_VERSION,
                             on_event=lambda msg: self.after(0, self.log, "API", msg))
        if self.settings.api_enabled:
            err = self.api.start()
            if err:
                self.after(0, self.log, "API", err, "warn")
        ...
        self.updater = AutoUpdater(
            ...
            on_event=lambda msg: self.after(0, self.log, "UPDATE", msg))
    ...
    def _after_scan(self, result) -> None:
        ...
        self.boost.auto_send(
            on_progress=lambda msg: self.after(0, self.log, "BOOST", msg),
